...
def test_the_allow_list_covers_every_platform_shape() -> None:
    ...
    # Exact members, not a substring check: CodeQL reads `"http://..." in value`
    # as an incomplete URL sanitizer.
    expected = {"tauri://localhost", "http://tauri.localhost", "http://localhost:1420"}
    assert expected <= set(DEFAULT_ALLOWED_ORIGINS)
